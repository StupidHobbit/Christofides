...
def make_graph_for_blossom(vertices, a):
    vertices = {v: Vertex(v) for v in vertices}
    for _u, _v in combinations(vertices, 2):
            w = a[_u][_v]
            u = vertices[_u]
            v = vertices[_v]
            u.add_edge_to(v, fractions.Fraction(w))
...
def blossom(vertices, a):
    global roots
    vertices = make_graph_for_blossom(vertices, a)
    ...
    d = list(tuple(v.id for v in e.vertices) for e in M)
    return d
